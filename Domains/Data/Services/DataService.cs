...
namespace SmartLab.Domains.Data.Services
{
    public class DataService : IDataService
    {
        ...
        public async Task<bool> MigrateFromJsonAsync(string jsonFilePath, string datasetDirectory)
        {
            try
            {
                if (!File.Exists(jsonFilePath))
                {
                    _logger.LogWarning("JSON file not found for migration: {FilePath}", jsonFilePath);
                    return false;
                }
                ...
                {
                    _logger.LogInformation("Empty JSON file, no datasets to migrate");
                    return true;
                }

                var legacyDatasets = JsonSerializer.Deserialize<List<Dataset>>(jsonContent);
                if (legacyDatasets == null) return true;

                foreach (var legacyDataset in legacyDatasets)
                {
                    var dataFileName = $"{legacyDataset.DatasetDate:yyyy-MM-dd-HH-mm-ss}_{legacyDataset.DatasetName}.txt";
                    var dataFilePath = Path.Combine(datasetDirectory, dataFileName);
                    
                    if (!File.Exists(dataFilePath))
                    {
                        _logger.LogWarning("Data file not found for dataset {DatasetId}: {FilePath}", 
                            legacyDataset.DatasetID, dataFilePath);
                        continue;
                    }

                    var dataLines = await File.ReadAllLinesAsync(dataFilePath);
                    
                    var dataset = new DatasetEntity
                    {
                        Id = legacyDataset.DatasetID,
                        Name = legacyDataset.DatasetName,
                        Description = legacyDataset.DatasetDiscription,
                        CreatedDate = legacyDataset.DatasetDate,
                ...
                _logger.LogInformation("Successfully migrated {Count} datasets from JSON", legacyDatasets.Count);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to migrate data from JSON file {FilePath}", jsonFilePath);
                throw;
            }
        }
    }
