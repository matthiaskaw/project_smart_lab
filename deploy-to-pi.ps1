...
ssh "${REMOTE_USER}@${REMOTE_HOST}" "pkill -f ${APP_NAME} 2>/dev/null || true; sleep 2; rm -rf ~/${APP_NAME} ~/temp_publish 2>/dev/null || true"
...
scp -r "${LOCAL_PUBLISH_DIR}" "${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_PATH}/temp_publish"
...
ssh "${REMOTE_USER}@${REMOTE_HOST}" "rm -rf ~/${APP_NAME} && mv ~/temp_publish ~/${APP_NAME} && chmod +x ~/${APP_NAME}/* 2>/dev/null || true && find ~/${APP_NAME} -type f -name 'smartlab' -exec chmod +x {} \;"
...
ssh "${REMOTE_USER}@${REMOTE_HOST}" "cd ~ && bash ~/startup.sh"
